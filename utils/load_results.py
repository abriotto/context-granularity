...
def load_accuracies(
    ...
    """
    Loads all accuracies into a dictionary.

    val_steps should be set to the same value as val_frequency during training.

    If granularities=True, this also loads:
      - granularity_coarse
      - granularity_fine
      - sampled_context

    Note:
      sampled_context is stored as:
        standard/sampled_context/<run>/

      not:
        standard/granularity_sampled_context/<run>/
    """
    ...
    result_dict = {
        ...
        'sampled_context_train_acc': [],
        'sampled_context_test_acc': [],
        'sampled_context_val_acc': [],
    ...
    for path_idx, path in enumerate(all_paths):
        ...
        # sampled_context is treated as another granularity condition,
        # but the folder is standard/sampled_context/, not standard/granularity_sampled_context/
        sampled_context_train_accs = []
        sampled_context_val_accs = []
        sampled_context_test_accs = []
        ...
        for run in range(n_runs):
            ...
            sampled_context_path = path + '/standard/sampled_context/' + str(run) + '/'
            ...
            # train and validation accuracy, standard/mixed condition
            ...
            if len(val_acc) > n_epochs // val_steps:
                val_acc = val_acc[::2]

            val_accs.append(val_acc)
            test_accs.append(data['final_test_acc'])

            # non-default granularity conditions
            if granularities and not zero_shot:
                ...
                lists = sorted(coarse_data['metrics_test0'].items())
                ...
                lists = sorted(fine_data['metrics_test0'].items())
                _, fine_val_acc = zip(*lists)

                if len(fine_val_acc) > n_epochs // val_steps:
                    fine_val_acc = fine_val_acc[::2]

                fine_val_accs.append(fine_val_acc)
                fine_test_accs.append(fine_data['final_test_acc'])

                # sampled_context
                sampled_context_data = pickle.load(open(sampled_context_path + 'loss_and_metrics.pkl', 'rb'))

                lists = sorted(sampled_context_data['metrics_train0'].items())
                _, sampled_context_train_acc = zip(*lists)
                sampled_context_train_accs.append(sampled_context_train_acc)

                lists = sorted(sampled_context_data['metrics_test0'].items())
                _, sampled_context_val_acc = zip(*lists)

                if len(sampled_context_val_acc) > n_epochs // val_steps:
                    sampled_context_val_acc = sampled_context_val_acc[::2]

                sampled_context_val_accs.append(sampled_context_val_acc)
                sampled_context_test_accs.append(sampled_context_data['final_test_acc'])

            if zero_shot:
                # TODO: implement loading of results for zero shot and fine/coarse/context settings,
                # if needed for further experiments
                if granularities:
        ...
        if granularities:
            ...
            result_dict['sampled_context_train_acc'].append(sampled_context_train_accs)
            result_dict['sampled_context_val_acc'].append(sampled_context_val_accs)
            result_dict['sampled_context_test_acc'].append(sampled_context_test_accs)
...
def load_entropies(
    ...
    Folder conventions:
      ...
      coarse/fine:
        standard/granularity_coarse/<run>/
        standard/granularity_fine/<run>/
    ...
    for path_idx, path in enumerate(all_paths):
        ...
        NMIs_conc_x_cont = []
        effectiveness_conc_x_cont = []
        consistency_conc_x_cont = []

        for run in range(n_runs):

            if granularity == 'mixed':
                entropy_path = path + '/' + setting + '/' + str(run) + '/'
            elif granularity == 'sampled_context':
                entropy_path = path + '/' + setting + '/sampled_context/' + str(run) + '/'
            else:
                entropy_path = path + '/' + setting + '/granularity_' + granularity + '/' + str(run) + '/'

            data = pickle.load(open(entropy_path + 'entropy_scores.pkl', 'rb'))
...
def load_accuracies_mu_and_goodman(
    ...
    """
    Loads all Mu and Goodman accuracies into a dictionary.

    val_steps should be set to the same as val_frequency during training.
    """
